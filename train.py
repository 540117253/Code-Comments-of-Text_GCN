...
flags.DEFINE_integer('hidden1', 15, 'Number of units in hidden layer 1.')
...
'''
    按照features.shape[0]=(train_size+vocab_size+test_size)的顺序设置one_hot编码，即图中结点的初始特征都为one_hot编码
    因此图中结点共有三类：训练集的doc结点、单词结点、测试集的doc结点
'''
features = sp.identity(features.shape[0])  # featureless，features此时为(train_size+vocab_size+test_size)*(train_size+vocab_size+test_size)的单位矩阵


# Some preprocessing
features = preprocess_features(features) # 对矩阵features的每一行都进行归一化处理，使得每一行元素之和为1
if FLAGS.model == 'gcn':
    support = [preprocess_adj(adj)] # support为D^(-1/2)*A*D^(-1/2)
    num_supports = 1
    model_func = GCN
...
#******************************************************************************#
#                    4.extract and save the doc/word embeddings                #
#******************************************************************************#

# doc and word embeddings
print('embeddings:')
word_embeddings = outs[3][train_size: adj.shape[0] - test_size] # outs[3]是第一层GCN的输出，shape=[(train_size+vocab_size+test_size)]
train_doc_embeddings = outs[3][:train_size]  # include val docs
test_doc_embeddings = outs[3][adj.shape[0] - test_size:] 

print(len(word_embeddings), len(train_doc_embeddings), len(test_doc_embeddings))
